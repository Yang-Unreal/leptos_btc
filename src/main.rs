...
// ============================================================================
// 客户端 / 非 SSR 版本的 main。
// #[cfg(not(feature = "ssr"))]：只有在“未开启 ssr 特性”（即编译浏览器端 WASM）时才编译它。
// 【为什么它是空的、为什么还必须存在】：
//   - 每个可执行目标(binary)都必须有一个 main 函数，否则链接不过。因此即使客户端用不到，
//     也得放一个 main“占位”，让编译能通过。
//   - 客户端真正的启动逻辑不在这里，而在 lib.rs 的 hydrate() 函数里（它带 #[wasm_bindgen]，
//     会被浏览器的 JS 调用，负责把服务器发来的静态 HTML “注水”成可交互页面）。
//     所以这个 main 什么都不用做。
// ============================================================================
#[cfg(not(feature = "ssr"))]
// 【为什么这里是普通 fn 而不是 #[tokio::main] async fn】：浏览器里没有 tokio 运行时，
//   也不需要它——异步调度由浏览器/JS 事件循环负责。因此客户端 main 就是个普通同步空函数。
pub fn main() {
    // 客户端无需在 main 里做任何事；启动逻辑见 lib.rs 的 hydrate()。
}
