...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route(
            ...
            // 下面逐层解释【为什么必须这样写】：
            //
            // 【Rust 基础语法讲解：闭包（Closure）】
            // 闭包就是"匿名函数"，写法是 |参数| { 函数体 }。
            // 它可以：
            //   - 捕获外部变量（如果没写 move，就是借用；写了 move，就是拿走所有权）
            //   - 自行推断参数和返回类型（不必写全类型标注）
            //   - 作为参数传给函数，或者赋值给变量
            //
            // 第 1 层：move |req: Request<Body>| { ... }
            //   req: Request<Body> 是闭包的参数，类型标注是"一个 HTTP 请求，body 是 Body 类型"。
            ...
            //   【Rust 基础语法讲解：move 关键字】
            //   move 告诉编译器："这个闭包里用到的外部变量，不要 borrow（借用），
            //   要把它们的 ownership（所有权）转移给闭包"。没有 move 的闭包默认按引用捕获，
            //   受限于外部变量的生命周期；move 闭包则完全自包含，可以脱离原作用域存活。
