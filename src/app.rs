...
// #[component]：声明这是一个 Leptos 组件。函数名 App 用大写开头，
// 【为什么大写】：view! 里区分“HTML 原生标签(小写，如 div)”和“自定义组件(大写，如 App)”
//   就是靠首字母大小写。
#[component]
...
pub fn Todos() -> impl IntoView {
    ...
    Effect::new(move |_| {
        ...
        // if let 是“模式匹配”：只在“有值(Some) 且 成功(Ok)”时才进入分支，拿到 list。
    ...
    view! {
        <section class="todos">
            ...
            <Transition fallback=move || view! { <p>"Loading todos…"</p> }>
                ...
                // match 同时看两个来源：todos（服务器 Resource）和 todos_local（本地镜像）。
